...
namespace EFHelper.RepositoryList
{
    public  class RepoListAsync : InterfaceRepoListAsync
    {
        ...
        public virtual IAsyncEnumerable<T> ListData<T>(List<SearchField> searchFieldList) where T : class
        {
            return list.QueryListGenerator<T>(searchFieldList, string.Empty, false, 0).ToAsyncEnumerable();
        }

        public virtual IAsyncEnumerable<T> ListData<T>(List<SearchField> searchFieldList, string sortColumn, bool isAscending, int topTake) where T : class
        {
            return list.QueryListGenerator<T>(searchFieldList, sortColumn,isAscending,topTake).ToAsyncEnumerable();
        }
        
        public virtual IAsyncEnumerable<TResult> ListData<TSource, TResult>(List<SearchField> searchFieldList, string sortColumn, bool isAscending, int topTake)
            where TSource : class
            where TResult : class
        {
